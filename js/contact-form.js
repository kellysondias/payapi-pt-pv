...
function checkName() {
  ...
  if (isEmpty(name)) {
    showError(nameInput, "Nome não pode estar em branco.");
    // Check if name input is at least 3 characters and no more than 30 characters
  } else if (!isBetween(name.length, min, max)) {
    showError(nameInput, `Nome precisa ter entre ${min} e ${max} caracteres`);
...
function checkEmail() {
  ...
  if (isEmpty(email)) {
    showError(emailInput, "E-mail não pode estar em branco");
...
function checkTitle() {
  ...
  if (isEmpty(title)) {
    showError(titleInput, "Título não pode estar em branco");
    // Check if title input is at least 2 characters and no more than 30 characters
  } else if (!isBetween(title.length, min, max)) {
    showError(titleInput, `Title precisa ter entre ${min} e ${max} caracteres`);
    // Otherwise, clear the error and set isValid to true
  } else {
